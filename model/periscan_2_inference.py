...
Input  : AnnData object with obs fields 'sample_id', 'annotation',
         and 'disease_1' (cancer type label)
Output : Per-sample top-k predictions and class probabilities
"""
...
# Class index mapping (alphabetical order, consistent with training)
IDX_TO_LABEL = {
    0: 'BLCA',
    1: 'CESC',
    2: 'COAD',
...
class PBMCInferenceDataset(Dataset):
    """
    Dataset for PERISCAN-II inference (no labels required).
...
def run_inference(
    ...
    gene_lists: dict,
    model_path: str,
    true_labels: dict = None,
    results_dir: str = '/results',
    batch_size: int = 8,
) -> pd.DataFrame:
    """
    Run PERISCAN-II inference on a PBMC AnnData object.
    ...
    gene_lists  : dict
                  ...
                  Periscan_2_genes.pkl.
    model_path  : str
                  Path to trained model checkpoint (.pt).
    true_labels : dict, optional
                  Mapping of sample_id -> cancer type string (from
                  obs['disease_1']). When provided, top-k accuracy
    ...
    results_df : pd.DataFrame
                 Columns: sample_id, top1_pred, top2_pred, top3_pred,
                          confidence, prob_BLCA, prob_CESC, prob_COAD,
                          prob_ESCA, prob_HNSC, prob_LIHC, prob_LUCA,
                          prob_NPC, prob_PAAD, prob_STAD
    """
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for i, sample_id in enumerate(batch['sample_ids']):
                row = {
                    'sample_id':  sample_id,
                    'top1_pred':  IDX_TO_LABEL[top3_idx[i, 0]],
                    'top2_pred':  IDX_TO_LABEL[top3_idx[i, 1]],
                    'top3_pred':  IDX_TO_LABEL[top3_idx[i, 2]],
                    'confidence': round(float(probs[i].max()), 4),
                }
                for j, cancer in IDX_TO_LABEL.items():
                    row[f'prob_{cancer}'] = round(float(probs[i, j]), 4)
                all_results.append(row)

    results_df = pd.DataFrame(all_results)

    # Optional: compute top-k accuracy if true labels provided
    if true_labels is not None:
        results_df['true_label'] = results_df['sample_id'].map(true_labels)
        top1 = (results_df['true_label'] == results_df['top1_pred']).mean()
        top2 = results_df.apply(
            lambda r: r['true_label'] in [r['top1_pred'], r['top2_pred']], axis=1
        ).mean()
        top3 = results_df.apply(
            lambda r: r['true_label'] in [r['top1_pred'], r['top2_pred'], r['top3_pred']], axis=1
        ).mean()

    # Save predictions
    out_path = os.path.join(results_dir, 'periscan_2_predictions.csv')
    results_df.to_csv(out_path, index=False)

    # Summary
    print("\n" + "=" * 60)
    print("PERISCAN-II Inference Results")
    print("=" * 60)
    print(results_df[['sample_id', 'top1_pred', 'top2_pred', 'top3_pred', 'confidence']].to_string(index=False))
    print(f"\nTop-1 prediction distribution:")
    print(results_df['top1_pred'].value_counts().to_string())
    if true_labels is not None:
        print(f"\nTop-k Accuracy:")
